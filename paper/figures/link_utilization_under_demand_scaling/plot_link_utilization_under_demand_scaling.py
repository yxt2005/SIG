    ...
    rows.append(
        {
            ...
            "actual_normal_u_link_max": float(row["actual_normal_u_link_max"]),
            "expected_fail_u_link_max": float(row["expected_fail_u_link_max"]),
            "source": source,
        }
    )
...
def write_plot_data(rows: list[dict[str, float | str]]) -> Path:
    out_path = OUT_DIR / "link_utilization_under_demand_scaling_data.csv"
...
def plot(rows: list[dict[str, float | str]]) -> tuple[Path, Path]:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.2, 3.35), sharex=True, constrained_layout=True)
    metrics = [
        ("actual_normal_u_link_max", "Actual normal link utilization"),
        ("expected_fail_u_link_max", "Expected failure-state link utilization"),
    ]
    ...
    axes[0].set_ylabel("Max link utilization")
    axes[0].set_ylim(0.35, 1.05)
    axes[1].set_ylim(0.35, 1.08)
    axes[1].axhline(1.0, color="#555555", linestyle=":", linewidth=1.2, zorder=0)
    axes[1].text(3.96, 1.01, "capacity", ha="right", va="bottom", fontsize=8, color="#555555")
    ...
    png_path = OUT_DIR / "link_utilization_under_demand_scaling.png"
    pdf_path = OUT_DIR / "link_utilization_under_demand_scaling.pdf"
